...
describe('Handler registrations are intercepted and altered', () => {
    ...
    describe('validation_override', () => {
        it('can override single "payload" validation properties', async () => {
            const res = await svc.test.put('/prefix/validation_override/2').send({
                description: 'test',
            });
            expect(res.statusCode).to.equal(200);
        });
        ...
        it('other validation keys are unchanged', async () => {
            const res = await svc.test.put('/prefix/validation_override/3').send({});
            expect(res.statusCode).to.equal(400);
        });

        it('can override "query" validation properties', async () => {
            const res = await svc.test.put('/prefix/validation_override/2?page=test').send({
                name: 'test',
                description: 'test',
            });
            expect(res.statusCode).to.equal(200);
        ...
        it('can override "params" validation properties', async () => {
            const res = await svc.test.put('/prefix/validation_override/string').send({
                name: 'test',
                description: 'test',
            });
            expect(res.statusCode).to.equal(200);
    ...
    describe('typeless parameters/schemas', () => {
        ['test', 0, true].forEach(v => {
            it(`can omit type in main parametes list send ?typless=${
        ...
        ['test', 0, {test: 'obj'}].forEach(v => {
            ...
            it(`Typless parameters ca be marked as required. Sending ${
                typeof v === 'object' ? JSON.stringify(v) : v
            }`, async () => {
