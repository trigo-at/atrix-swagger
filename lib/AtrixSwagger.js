...
const getParams = filter(propEq('in', 'path')); // eslint-disable-line
const getQuery = filter(propEq('in', 'query')); // eslint-disable-line
const getHeaders = filter(propEq('in', 'header')); // eslint-disable-line
const getBody = filter(propEq('in', 'body')); // eslint-disable-line
	...
	async setupServiceHandler({ method, path, config }) {
		...
		if (routeSpecs.parameters) {
			...
			if (headerParameters.length) {
				let swaggerHeadersSchema = this.createParameterValidator(getHeaders(routeSpecs.parameters));
