...
describe('createParameterValidation', () => {
    function getSchema(def) {
        ...
        return {
            schema,
            validate: (obj) => schema.validate(obj),
            ok: obj => {
    ...
    it('can omit attribute "type" to allow any value', () => {
        ...
        schema.ok({ test: 1.2 });
        schema.ok({ test: [1, '12'] });
        schema.ok({ test: { prop: 1, sub: { p: '12' } } });
    });
    ...
    it('handles attribute "default"', () => {
        ...
        const parsedValue = schema.validate({}); //   Joi.validate({}, schema.schema);
        expect(parsedValue.value.test).to.equal('uuid');
    });

    it('handles attribute "nullable"', () => {
        ...
        const schema3 = getSchema({
            name: 'test',
            type: 'string',
        });

        schema1.ok({ test: 'asdf' });
        schema1.ok({ test: null });
        schema1.fail({ test: undefined });
        schema2.fail({ test: null });
        schema3.fail({ test: null });
    });
    ...
    describe('"number" properties & options', () => {
        ...
        it('handles "minimum" attribute', () => {
            ...
            schema.ok({ test: 1030 });
            schema.ok({ test: 42 });
            schema.fail({ test: -14 }, /must be greater than or equal/);
            schema.fail({ test: 41.99999999 }, /must be greater than or equal/);
        });
        ...
        it('handles attribute "default"', () => {
            ...
            const parsedValue = schema.validate({});
    ...
    describe('"string" properties & options', () => {
        ...
        it('string validator allows empty strings', () => {
            const schema = getSchema({
                name: 'test',
                type: 'string',
            });
        ...
        it('empty string not allowed with "pattern" attribute', () => {
            const schema = getSchema({
                name: 'test',
                type: 'string',
                pattern: '^start',
        ...
        it('empty string not allowed with minLength attribute', () => {
            const schema = getSchema({
                name: 'test',
                type: 'string',
                minLength: 1,
    ...
    describe('array properties', () => {
        ...
        it('handles empty arrays with "required" attribute', () => {
            const schema = getSchema({
                name: 'test',
                type: 'array',
                items: {
            ...
            schema.ok({ test: [{ id: '1', name: 'obj1' }, { id: '2', name: 'obj2' }] });
            schema.ok({ test: [] });
            schema.fail({ test: [{ id: '1' }] }, /is required/);
        });

        it('handles arrays with "required" object items', () => {
            const schema = getSchema({
                name: 'test',
                type: 'array',
                items: {
            ...
            schema.ok({ test: [{ id: '1', name: 'obj1' }, { id: '2', name: 'obj2' }] });
            schema.ok({ test: [{ id: '1' }] });
            schema.fail({ test: [] }, /does not contain 1 required/);
        });

        it('in query: array can also be a single value', () => {
            const schema = getSchema({
                in: 'query',
                name: 'test',
                type: 'array',
    ...
    describe('"object" properties', () => {
        ...
        it('object type property', () => {
            ...
            schema.ok({ test: {} });
            schema.fail({ test: { name: { id: -1 } } }, /must be greater than or equal to 0/);
        });
        ...
        it('"additionalProperties" = true allows undeclared properties', () => {
            const schema = getSchema({
                name: 'test',
                type: 'object',
                properties: {
            ...
            schema.ok({ test: { name: 'AtrixSwagger' } });
            schema.ok({ test: { name: 'AtrixSwagger', ndn: 'asdf', asd: { a: [1, 3] } } });
            schema.fail({ test: {} }, /is required/);
        });
    });
});
